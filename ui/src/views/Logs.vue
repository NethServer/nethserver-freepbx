<template>
  <div>
    <h1 class="app-title">{{$t('logs.title')}}</h1>
    <h3>
      <pre id="log-file"
        class="monospace m-right-sm">{{!view.follow ? 'tail -'+view.lines+' /var/log/messages' : 'tail -f /var/log/messages'}}</pre>
      <button @click="handleLogs()"
        class="btn btn-primary">{{view.follow ? $t('logs.stop_follow') : $t('logs.follow')}}</button>
    </h3>
